...
export default function Home({
    ...
    const [unseenPage, setUnseenPage] = useState(
        unseenPagination?.current_page || 1,
    );
    const [savedPage, setSavedPage] = useState(
        savedPagination?.current_page || 1,
    );
    const [allVideos, setAllVideos] = useState<Video[]>(videos.all || []);
    const [unseenVideos, setUnseenVideos] = useState<Video[]>(
        videos.unseen || [],
    );
    const [savedVideos, setSavedVideos] = useState<Video[]>(videos.saved || []);
    const [isLoadingMore, setIsLoadingMore] = useState(false);
    const [unseenHasMore, setUnseenHasMore] = useState(
        unseenPagination?.has_more || false,
    );
    const [savedHasMore, setSavedHasMore] = useState(
        savedPagination?.has_more || false,
    );
    ...
    const handleSeenToggle = (videoId: number, isSeen: boolean) => {
        ...
        if (isSeen) {
            ...
            if (video) {
                setUnseenVideos((prev) =>
                    [{ ...video, is_seen: false }, ...prev].sort(
                        (a, b) =>
    ...
    const handleSaveToggle = (videoId: number, isSaved: boolean) => {
        ...
        if (isSaved) {
            const video =
                allVideos.find((e) => e.id === videoId) ||
                unseenVideos.find((e) => e.id === videoId);
            if (video) {
                setSavedVideos((prev) => [
                    { ...video, is_saved: true },
                    ...prev,
    ...
    const loadMoreVideos = () => {
        if (!pagination?.has_more || isLoadingMore || activeTab !== 'all')
            return;
