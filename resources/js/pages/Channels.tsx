...
export default function Channels({ channels: channelsData, categories, stats }: ChannelsProps) {
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      ...
      <div className="container mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card className="transition-all duration-200 hover:shadow-md hover:-translate-y-1">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Unseen</CardTitle>
              <Eye className="h-4 w-4 text-muted-foreground transition-transform group-hover:scale-110" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{stats.unseenCount}</div>
              <p className="text-xs text-muted-foreground">
                Items to read
              </p>
            </CardContent>
